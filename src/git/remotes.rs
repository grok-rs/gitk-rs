...
use anyhow::Result;
use git2::{FetchOptions, Progress, PushOptions, RemoteCallbacks, Repository};
...
impl RemoteManager {
    ...
    pub fn set_credentials_provider(
        &mut self,
        provider: Box<dyn CredentialsProvider + Send + Sync>,
    ) {
    ...
    pub fn fetch(
        ...
        if let Some(ref provider) = self.credentials_provider {
            ...
            callbacks.credentials(
                    ...
                    .get_credentials(url, username_from_url)
                {
                    ...
                    Ok(Credentials::Token(token)) => git2::Cred::userpass_plaintext(&token, ""),
                    Ok(Credentials::Default) => git2::Cred::default(),
        ...
        let refspecs: Vec<String> = if config.refspecs.is_empty() {
            ...
            (0..stringarray.len())
                .filter_map(|i| stringarray.get(i).map(|s| s.to_string()))
                .collect()
        } else {
        ...
        if fetch_success {
            self.record_operation(OperationRecord {
                ...
                description: format!(
                    "Fetched from remote '{}' ({} refs updated)",
                    sanitized_name,
                    ref_updates.len()
                ),
                original_state: None,
            ...
            info!(
                "Successfully fetched from remote '{}': {} objects, {} refs updated",
                sanitized_name,
                transfer_stats.received_objects,
                ref_updates.len()
            );
        }
        ...
        Ok(RemoteOperationResult {
            ...
            message: if fetch_success {
                format!(
                    "Fetch completed: {} objects received",
                    transfer_stats.received_objects
                )
            } else {
                "Fetch failed".to_string()
            },
            transferred_objects: if fetch_success {
                Some(transfer_stats)
            } else {
                None
            },
    ...
    pub fn push(&mut self, remote_name: &str, config: PushConfig) -> Result<RemoteOperationResult> {
        ...
        if push_success {
            ...
            info!(
                "Successfully pushed to remote '{}': {} refs updated",
                sanitized_name,
                push_updates.len()
            );
        }
    ...
    pub fn pull(&mut self, remote_name: &str, config: PullConfig) -> Result<RemoteOperationResult> {
        ...
        if integration_success {
            self.record_operation(OperationRecord {
                ...
                timestamp: chrono::Utc::now(),
                description: format!(
                    "Pulled from remote '{}' and integrated changes",
                    remote_name
                ),
                original_state: None,
                new_state: Some("integrated".to_string()),
                affected_refs: fetch_result
                    .updated_refs
                    .iter()
            ...
            info!(
                "Successfully pulled from remote '{}' and integrated changes",
                remote_name
            );
        }
    ...
    pub fn add_remote(&mut self, name: &str, url: &str) -> Result<RemoteOperationResult> {
        ...
        if add_success {
            self.record_operation(OperationRecord {
                ...
                timestamp: chrono::Utc::now(),
                description: format!(
                    "Added remote '{}' with URL '{}'",
                    sanitized_name, sanitized_url
                ),
                original_state: None,
            ...
            info!(
                "Successfully added remote '{}' with URL '{}'",
                sanitized_name, sanitized_url
            );
        }
