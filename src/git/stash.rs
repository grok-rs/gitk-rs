...
use anyhow::Result;
use git2::{Repository, Signature, StashApplyOptions, StashFlags};
...
impl StashManager {
    ...
    pub fn create_stash(&mut self, config: StashCreateConfig) -> Result<StashOperationResult> {
        ...
        let message = if let Some(ref msg) = config.message {
            ...
            format!(
                ...
                self.get_latest_commit_summary()
    ...
    pub fn apply_stash(
        ...
        if stash_index >= stash_count {
            return Ok(StashOperationResult {
                ...
                stash_id: None,
                message: format!(
                    "Stash index {} out of range (0-{})",
                    stash_index,
                    stash_count.saturating_sub(1)
                ),
                conflicts: vec![],
                modified_files: vec![],
                stash_info: None,
            ...
            if let Some(conflicts) = self.check_stash_conflicts(stash_index)? {
                if !conflicts.is_empty() {
                    return Ok(StashOperationResult {
                        ...
                        message: format!(
                            "Applying stash would cause {} conflicts",
                            conflicts.len()
                        ),
        ...
        if success {
            ...
            info!(
                "Successfully applied stash {}: {}",
                stash_index, stash_info.message
            );
        } else {
    ...
    pub fn pop_stash(
        ...
        if !drop_result.success {
            warn!(
                "Stash applied but failed to remove: {}",
                drop_result.message
            );
            return Ok(StashOperationResult {
                operation: OperationType::StashPop,
        ...
        for conflict in conflict_iter {
            if let Ok(conflict_data) = conflict {
                ...
                let conflict_info = StashConflict {
                    path,
                    conflict_type: self.classify_stash_conflict_type(
