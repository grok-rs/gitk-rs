...
use anyhow::Result;
use git2::{CherrypickOptions, Oid, Repository, ResetType, RevertOptions};
...
impl CommitOperations {
    ...
    pub fn cherry_pick(
        ...
        let cherry_pick_result = {
            ...
            match self
                .repo
                .cherrypick(&target_commit, Some(&mut cherrypick_opts))
            {
                Ok(()) => {
                    // Check for conflicts
        ...
        if success {
            self.record_operation(OperationRecord {
                ...
                affected_refs: if new_commit_id.is_some() {
                    vec!["HEAD".to_string()]
                } else {
                    vec![]
                },
            ...
            info!(
                ...
                sanitized_commit, new_commit_id
            );
        } else {
        ...
        Ok(CommitOperationResult {
            ...
            new_commit_id,
            message: if success {
                "Cherry-pick successful".to_string()
            } else {
                error_message
            },
            conflicts,
    ...
    pub fn reset(&mut self, commit_id: &str, config: ResetConfig) -> Result<CommitOperationResult> {
        ...
        let reset_result = {
            ...
            match self.repo.reset(target_commit.as_object(), reset_type, None) {
                ...
                Err(e) => (false, String::new(), vec![], format!("Reset failed: {}", e)),
            }
        };
        ...
        Ok(CommitOperationResult {
            ...
            new_commit_id,
            message: if success {
                format!("Reset successful ({:?})", config.reset_type)
            } else {
                error_message
            },
            conflicts: vec![],
    ...
    fn configure_cherrypick_strategy(
        &self,
        _cherrypick_opts: &mut CherrypickOptions,
        ...
        match strategy {
            ...
            _ => {
                ...
                warn!(
                    "Merge strategy {:?} not fully supported, using default",
                    strategy
                );
            }
        }
    ...
    fn configure_revert_strategy(
        &self,
        _revert_opts: &mut RevertOptions,
        strategy: &MergeStrategy,
    ) {
        match strategy {
            MergeStrategy::Recursive => {
    ...
    fn create_cherry_pick_commit(
        ...
        let mut message = original_commit
            .message()
...
mod tests {
    ...
    fn test_cherry_pick_operation() -> Result<()> {
        ...
        let feature_commit = create_test_commit(
            &repo_path,
            "feature.txt",
            "feature content",
            "Feature commit",
    ...
    fn test_revert_operation() -> Result<()> {
        ...
        let commit_to_revert =
            create_test_commit(&repo_path, "file2.txt", "content2", "Second commit")?;
    ...
    fn test_reset_types() -> Result<()> {
        ...
        let reset_types = [GitResetType::Soft, GitResetType::Mixed, GitResetType::Hard];
    ...
    fn test_error_handling() -> Result<()> {
        ...
        let result = operations.cherry_pick(
            "1234567890123456789012345678901234567890",
            CherryPickConfig::default(),
        );
        assert!(result.is_err());
