...
impl InputValidator {
    ...
    pub fn validate_ref_name(name: &str) -> Result<(), GitError> {
            ...
            || name.contains("@{")
        {
            return Err(GitError::invalid_input(
                name,
                "Reference name violates Git naming rules",
            ));
        }
    ...
    pub fn validate_commit_message(message: &str) -> Result<(), GitError> {
        ...
        if message.len() > 8192 {
            return Err(GitError::invalid_input(
                message,
                "Commit message too long (maximum 8192 characters)",
            ));
        }
    ...
    pub fn validate_file_path(path: &str) -> Result<(), GitError> {
        ...
        if path.len() > 4096 {
            return Err(GitError::invalid_input(
                path,
                "File path too long (maximum 4096 characters)",
            ));
        }
        ...
        if path.starts_with('/')
            || (cfg!(windows) && path.len() > 2 && path.chars().nth(1) == Some(':'))
        {
    ...
    pub fn validate_command_args(args: &[&str]) -> Result<(), GitError> {
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            for ch in &dangerous_chars {
                if arg.contains(*ch) {
                    return Err(GitError::security(format!(
    ...
    pub fn user_friendly_message(error: &GitError) -> String {
        match error {
            ...
            GitError::PermissionDenied { resource, .. } => {
                format!(
                    "Permission denied: Cannot access {}. Please check file permissions.",
                    resource
                )
            }
            ...
            GitError::CommandExecution { command, .. } => {
                format!(
                    "Command '{}' failed. Please check the repository state.",
                    command
                )
            }
            GitError::Repository { .. } => {
            ...
            GitError::ResourceLimit { resource, .. } => {
                format!(
                    "Resource limit exceeded for {}. Please reduce the operation scope.",
                    resource
                )
            }
            GitError::Git2(e) => {
