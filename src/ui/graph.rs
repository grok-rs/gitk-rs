...
impl CommitGraphRenderer {
    ...
    fn compute_graph_layout(
        ...
        for (row, commit) in commits.iter().enumerate() {
            ...
            let parent_lines =
                self.create_parent_lines(commit, pos, &lane_assignments, &parent_map, row, commits);

            // Create child connection lines
            let child_lines =
                self.create_child_lines(commit, pos, &lane_assignments, &child_map, row, commits);
            ...
            layout
                .commit_positions
                .insert(commit.id.clone(), commit_pos);
        }
        ...
        layout.total_width = (lane_assignments.values().max().unwrap_or(&0) + 1) as f32
            * self.column_width
            * self.zoom_level;
                ...
                if let Some(parent_lane) = lane_assignments.get(parent_id) {
                    ...
                    if let Some(parent_row) = commits.iter().position(|c| &c.id == parent_id) {
                        let parent_pos = egui::Pos2 {
                            x: (*parent_lane as f32) * self.column_width * self.zoom_level
                                + self.pan_offset.x,
                            y: (parent_row as f32) * self.row_height * self.zoom_level
                                + self.pan_offset.y,
                        };
                ...
                if let Some(child_lane) = lane_assignments.get(child_id) {
                    if let Some(child_row) = commits.iter().position(|c| &c.id == child_id) {
                        let child_pos = egui::Pos2 {
                            x: (*child_lane as f32) * self.column_width * self.zoom_level
                                + self.pan_offset.x,
                            y: (child_row as f32) * self.row_height * self.zoom_level
                                + self.pan_offset.y,
                        };
    ...
    fn create_merge_lines(
        &self,
        commits: &[GitCommit],
        positions: &HashMap<String, CommitPosition>,
    ) -> Vec<MergeLine> {
    ...
    fn draw_connection_line(&self, painter: &egui::Painter, line: &ConnectionLine) {
        ...
        if line.control_points.is_empty() {
            ...
            for i in 0..=10 {
                let t = i as f32 / 10.0;
                let point = self.cubic_bezier(
                    line.start,
                    line.control_points[0],
                    line.control_points[1],
                    line.end,
                    t,
                );
                path.push(point);
    ...
    fn draw_commit(&self, painter: &egui::Painter, commit_pos: &CommitPosition, is_hovered: bool) {
        ...
        if self
            .interaction_state
            .selected_commits
            .contains(&commit_pos.pos.to_string())
        {
    ...
    fn draw_merge_indicator(&self, painter: &egui::Painter, merge_line: &MergeLine) {
        ...
        match merge_line.style {
            ...
            MergeStyle::Octopus => {
                ...
                // Add octopus label
                let text_pos =
                    center + egui::Vec2::new(10.0 * self.zoom_level, -15.0 * self.zoom_level);
                painter.text(
                    text_pos,
            ...
            MergeStyle::Highlighted => {
                ...
                for (i, parent_pos) in merge_line.parent_positions.iter().enumerate() {
                    ...
                    for glow_radius in [6.0, 4.0, 2.0] {
                        ...
                        let glow_color =
                            egui::Color32::from_rgba_unmultiplied(255, 255, 0, glow_alpha);
                        let glow_stroke =
                            egui::Stroke::new(glow_radius * self.zoom_level, glow_color);
                        painter.line_segment([center, *parent_pos], glow_stroke);
                    }

                    // Draw main highlighted line
                    let highlight_stroke =
                        egui::Stroke::new(4.0 * self.zoom_level, egui::Color32::YELLOW);
    ...
    fn draw_ref_label(
        ...
        let text_galley =
            painter.layout_no_wrap(ref_label.text.clone(), font.clone(), ref_label.text_color);

        let background_rect =
            egui::Rect::from_min_size(label_pos, text_galley.size() + egui::Vec2::splat(4.0));
        ...
        // Draw text
        painter.galley(
            label_pos + egui::Vec2::splat(2.0),
            text_galley,
            ref_label.text_color,
        );
    }

    /// Draw highlighted path through commit history
    fn draw_highlighted_path(
        &self,
        painter: &egui::Painter,
        path: &[String],
        layout: &GraphLayout,
    ...
    fn classify_branch_type(&self, branch_name: &str, commits: &[GitCommit]) -> BranchType {
        ...
        if name_lower.starts_with("release/")
            || name_lower.starts_with("rel/")
        ...
        if name_lower.starts_with("hotfix/")
            || name_lower.starts_with("fix/")
        ...
        if name_lower.starts_with("feature/")
            || name_lower.starts_with("feat/")
            || name_lower.starts_with("dev/")
        ...
        match branch_type {
            ...
            BranchType::Feature => {
                ...
                let recent_commits = commits
                    ...
                    .filter(|c| {
                        c.message.contains(branch_name) || c.author.name.contains(branch_name)
                    })
                    .count();
    ...
    fn assess_merge_conflict_risk(
        ...
        for parent_id in &merge_commit.parent_ids {
            if let Some(parent_commit) = commits.iter().find(|c| &c.id == parent_id) {
                let age_diff =
                    merge_commit.author.when.timestamp() - parent_commit.author.when.timestamp();
                parent_ages.push(age_diff);
            }
        }

        if let Some(&max_age_diff) = parent_ages.iter().max() {
            if max_age_diff > 86400 * 7 {
                // More than a week apart
                ConflictSeverity::Medium
            } else if max_age_diff > 86400 * 30 {
        ...
        for _ in 0..20 {
            ...
            if let Some(commit_pos) = layout.commit_positions.get(&current_id) {
                ...
                if let Some(first_parent_line) = commit_pos.parent_lines.first() {
                    ...
                    if let Some((parent_id, _)) = layout
                        .commit_positions
                        .iter()
    ...
    fn trace_commit_ancestry(&mut self, commit_id: &str, layout: &GraphLayout) {
        ...
        self.highlighted_path = if ancestry_path.len() > 1 {
            Some(ancestry_path)
        } else {
            None
        };
    ...
    fn fit_to_window(&mut self, layout: &GraphLayout, window_rect: egui::Rect) {
        ...
        if content_width > 0.0 && content_height > 0.0 {
            ...
            self.pan_offset = window_center.to_vec2()
                - egui::Vec2::new(content_center_x, content_center_y) * self.zoom_level;
        }
    ...
    fn render_interaction_hints(&self, ui: &mut egui::Ui, rect: egui::Rect) {
        ...
        if !self.interaction_state.selected_commits.is_empty() {
            let selection_text = format!(
                "📌 {} selected",
                self.interaction_state.selected_commits.len()
            );
