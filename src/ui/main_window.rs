...
use crate::ui::{CommitGraph, DiffViewer, SearchPanel, ReferencesPanel, ViewsPanel};
...
impl MainWindow {
    ...
    fn show_menu_bar(&mut self, ui: &mut egui::Ui, state: &mut AppState, _config: &AppConfig) {
        egui::menu::bar(ui, |ui| {
            ...
            ui.menu_button("View", |ui| {
                ...
                ui.radio_value(&mut self.layout_mode, LayoutMode::TwoPaneHorizontal, "Two Pane Horizontal");
                ui.radio_value(&mut self.layout_mode, LayoutMode::TwoPaneVertical, "Two Pane Vertical");
                ui.radio_value(&mut self.layout_mode, LayoutMode::ThreePaneClassic, "Three Pane Classic");
    ...
    fn show_toolbar(&mut self, ui: &mut egui::Ui, state: &mut AppState, config: &AppConfig) {
        ui.horizontal(|ui| {
            ...
            // Layout buttons
            if ui.button("1").on_hover_text("Single Pane (Ctrl+1)").clicked() {
                self.layout_mode = LayoutMode::SinglePane;
            }
            if ui.button("2").on_hover_text("Two Pane (Ctrl+2)").clicked() {
                self.layout_mode = LayoutMode::TwoPaneHorizontal;
            }
            if ui.button("3").on_hover_text("Three Pane (Ctrl+3)").clicked() {
                self.layout_mode = LayoutMode::ThreePaneClassic;
            }
            ...
            // Panel toggles
            if ui.button("📊").on_hover_text("Toggle Commit Graph").clicked() {
                self.panel_visibility.commit_graph = !self.panel_visibility.commit_graph;
            }
            if ui.button("🌳").on_hover_text("Toggle File Tree (Ctrl+T)").clicked() {
                self.panel_visibility.file_tree = !self.panel_visibility.file_tree;
            }
            if ui.button("🏷️").on_hover_text("Toggle References (Ctrl+R)").clicked() {
                self.panel_visibility.references = !self.panel_visibility.references;
            }
            ...
            if self.panel_visibility.search {
                ...
                if ui.button("🔍").on_hover_text("Show Search (Ctrl+F)").clicked() {
                    self.panel_visibility.search = true;
                }
            }
    ...
    fn show_three_pane_layout(&mut self, ui: &mut egui::Ui, state: &mut AppState, config: &AppConfig) {
    ...
    fn show_two_pane_horizontal_layout(&mut self, ui: &mut egui::Ui, state: &mut AppState, config: &AppConfig) {
    ...
    fn show_two_pane_vertical_layout(&mut self, ui: &mut egui::Ui, state: &mut AppState, config: &AppConfig) {
        // Optional side panels
        if self.panel_visibility.references {
            egui::SidePanel::left("references")
                .resizable(true)
    ...
    fn show_single_pane_layout(&mut self, ui: &mut egui::Ui, state: &mut AppState, config: &AppConfig) {
                            ...
                            for entry in entries {
                                ...
                                if response.clicked() {
                                    ...
                                    if !entry.is_tree {
                                        ...
                                        if let Ok(diff) = repo.get_file_diff(&selected_commit.id, &entry.path) {
