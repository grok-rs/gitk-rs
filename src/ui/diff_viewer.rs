...
use crate::models::{GitDiff, GitDiffLine, DiffStatus};
    ...
    fn insert_file(&mut self, path_parts: Vec<&str>, file_idx: usize, _diff: &GitDiff) {
        ...
        for (i, part) in path_parts.iter().enumerate() {
            if i == path_parts.len() - 1 {
                ...
                current.children.insert(part.to_string(), FileTreeNode::new_file(file_idx));
            } else {
                // Intermediate directories
                let part_string = part.to_string();
                current.children.entry(part_string.clone()).or_insert_with(FileTreeNode::new_dir);
...
impl DiffViewer {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                egui::ComboBox::from_label("View")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        ui.selectable_value(&mut self.view_mode, DiffViewMode::SideBySide, "Side by Side");
                        ui.selectable_value(&mut self.view_mode, DiffViewMode::Split, "Split View");
                        ui.selectable_value(&mut self.view_mode, DiffViewMode::InlineChanges, "Inline Changes");
                    });
                ...
                if !self.search_matches.is_empty() {
                    ui.label(format!("{}/{}", self.current_match + 1, self.search_matches.len()));
                    if ui.button("⏪").clicked() && self.current_match > 0 {
                        self.current_match -= 1;
                    }
                    if ui.button("⏩").clicked() && self.current_match < self.search_matches.len() - 1 {
                        self.current_match += 1;
                    }
                }
            });
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            for (file_idx, diff) in diffs.iter().enumerate() {
                ...
                for (hunk_idx, hunk) in diff.hunks.iter().enumerate() {
                    ...
                    if !self.folded_hunks.get(&(file_idx * 1000 + hunk_idx)).unwrap_or(&false) {
    ...
    fn show_inline_changes_view(&mut self, ui: &mut egui::Ui, diffs: &[GitDiff], _state: &AppState) {
        egui::ScrollArea::vertical().show(ui, |ui| {
            for (file_idx, diff) in diffs.iter().enumerate() {
                self.show_file_header(ui, diff, file_idx);
    ...
    fn show_binary_file_info(&self, ui: &mut egui::Ui, diff: &GitDiff) {
        ui.indent("binary_info", |ui| {
            ui.colored_label(egui::Color32::LIGHT_GRAY, "📁 Binary file - content not shown");
    ...
    fn show_hunk_header(&self, ui: &mut egui::Ui, hunk: &crate::models::GitHunk, file_idx: usize, hunk_idx: usize) {
        ui.horizontal(|ui| {
            // Fold/unfold button
        ...
        ui.allocate_new_ui(egui::UiBuilder::new().max_rect(rect), |ui| {
            ui.horizontal(|ui| {
                ...
                if self.show_line_numbers {
                    let old_num = line.old_lineno.map(|n| n.to_string()).unwrap_or_else(|| "   ".to_string());
                    let new_num = line.new_lineno.map(|n| n.to_string()).unwrap_or_else(|| "   ".to_string());
        ...
        ui.allocate_new_ui(egui::UiBuilder::new().max_rect(rect), |ui| {
            ui.horizontal(|ui| {
                ...
                if self.show_line_numbers {
                    ...
                    let num_str = line_num.map(|n| n.to_string()).unwrap_or_else(|| "   ".to_string());
    ...
    fn show_inline_change_line(&self, ui: &mut egui::Ui, line: &DiffLine) {
        ...
        ui.allocate_new_ui(egui::UiBuilder::new().max_rect(rect), |ui| {
            ui.horizontal(|ui| {
                // Line numbers
                if self.show_line_numbers {
                    let old_num = line.old_line_no.map(|n| n.to_string()).unwrap_or_else(|| "   ".to_string());
                    let new_num = line.new_line_no.map(|n| n.to_string()).unwrap_or_else(|| "   ".to_string());
                    ui.monospace(format!("{:>4} {:>4}", old_num, new_num));
                    ui.separator();
                }
                ...
                let content = line.new_line.as_ref().or(line.old_line.as_ref()).unwrap_or(&default_content);
    ...
    fn show_syntax_highlighted_text(&self, ui: &mut egui::Ui, text: &str, base_color: egui::Color32) {
    ...
    fn is_line_search_match(&self, line: &GitDiffLine) -> bool {
        ...
        line.content.to_lowercase().contains(&self.search_text.to_lowercase())
    }
    ...
    fn show_file_tree_node(&mut self, ui: &mut egui::Ui, node: &FileTreeNode, path: &str, diffs: &[GitDiff]) {
            ...
            if child_node.is_directory() {
                ...
                ui.horizontal(|ui| {
                    if ui.button(expand_symbol).clicked() {
                        self.file_tree_expanded.insert(full_path.clone(), !is_expanded);
                    }
                    ui.label(name);
                });
    ...
    fn show_file_navigation_bar(&mut self, ui: &mut egui::Ui, diffs: &[GitDiff]) {
        ui.horizontal(|ui| {
            ...
            if ui.button("⏪").on_hover_text("Previous file").clicked() && self.current_file_index > 0 {
                self.current_file_index -= 1;
            }
            ...
            egui::ComboBox::from_id_source("file_selector")
                ...
                .show_ui(ui, |ui| {
                    for (idx, diff) in diffs.iter().enumerate() {
                        ...
                        if ui.selectable_label(is_selected, format!("{} {}", icon, file_name)).clicked() {
                            self.current_file_index = idx;
                        }
                    }
                });
            
            if ui.button("⏩").on_hover_text("Next file").clicked() && self.current_file_index < diffs.len() - 1 {
                self.current_file_index += 1;
            }
...
impl SyntaxHighlighter {
    ...
    fn add_rust_highlighter(&mut self) {
        ...
        self.language_patterns.insert("rs".to_string(), highlighter.clone());
        self.language_patterns.insert("rust".to_string(), highlighter);
    }
    ...
    fn add_python_highlighter(&mut self) {
        ...
        self.language_patterns.insert("py".to_string(), highlighter.clone());
        self.language_patterns.insert("python".to_string(), highlighter);
    }
    ...
    fn add_javascript_highlighter(&mut self) {
        ...
        let patterns = vec![
            ...
            // Functions
            SyntaxPattern::new(r"\bfunction\s+([a-zA-Z_$][a-zA-Z0-9_$]*)", TokenType::Function, 2),
            SyntaxPattern::new(r"\b([a-zA-Z_$][a-zA-Z0-9_$]*)\s*\(", TokenType::Function, 1),
        ...
        self.language_patterns.insert("js".to_string(), highlighter.clone());
        self.language_patterns.insert("javascript".to_string(), highlighter.clone());
        
        // TypeScript (extends JavaScript)
        let mut ts_keywords = keywords;
        ts_keywords.extend(vec![
            "type", "interface", "namespace", "module", "declare", "public", "private", "protected",
            "readonly", "abstract", "static", "implements", "extends", "keyof", "infer", "is",
        ...
        self.language_patterns.insert("ts".to_string(), ts_highlighter.clone());
        self.language_patterns.insert("typescript".to_string(), ts_highlighter);
    }
    ...
    fn add_common_language_highlighters(&mut self) {
        ...
        self.language_patterns.insert("json".to_string(), json_highlighter);
        ...
        self.language_patterns.insert("md".to_string(), md_highlighter.clone());
        self.language_patterns.insert("markdown".to_string(), md_highlighter);
    }
    ...
    fn tokenize_with_highlighter(&self, text: &str, highlighter: &LanguageHighlighter) -> Vec<SyntaxToken> {
...
impl WordDiffEngine {
    ...
    fn compute_word_diff(&mut self, old_line: &str, new_line: &str) -> WordDiffResult {
        ...
        let result = WordDiffResult {
            old_words: old_words.into_iter().map(|(text, start, end)| DiffWord {
                text,
                start_pos: start,
                end_pos: end,
                change_type: WordChangeType::Removed, // Will be updated based on operations
            }).collect(),
            new_words: new_words.into_iter().map(|(text, start, end)| DiffWord {
                text,
                start_pos: start,
                end_pos: end,
    ...
    fn compute_diff_operations(&self, old_words: &[(String, usize, usize)], new_words: &[(String, usize, usize)]) -> Vec<DiffOperation> {
        ...
        while old_idx < old_texts.len() || new_idx < new_texts.len() {
            if lcs_idx < lcs.len() && old_idx < old_texts.len() && new_idx < new_texts.len() 
                && old_texts[old_idx] == lcs[lcs_idx] && new_texts[new_idx] == lcs[lcs_idx] {
                // Common text
                operations.push(DiffOperation::Equal(old_texts[old_idx].to_string()));
                old_idx += 1;
                new_idx += 1;
                lcs_idx += 1;
            } else if old_idx < old_texts.len() && (lcs_idx >= lcs.len() || old_texts[old_idx] != lcs[lcs_idx]) {
                // Deletion
                if new_idx < new_texts.len() && (lcs_idx >= lcs.len() || new_texts[new_idx] != lcs[lcs_idx]) {
                    // Replacement
